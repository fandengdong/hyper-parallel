...
class _TransformerBlock(nn.Module):
    ...
    def __init__(self, dim=256, num_heads=4, swap_tensor_on=False, group_swap=False, cpu_pool=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.swap_tensor_on:
            x = swap_tensor_wrapper(x, tag="attn_residual", group_swap=self.group_swap, cpu_pool=self.cpu_pool)
        x = self.norm1(x)
        if self.swap_tensor_on:
            x = swap_tensor_wrapper(x, tag="norm1_out", group_swap=self.group_swap, cpu_pool=self.cpu_pool)
        x = x + self.ffn(x)
        if self.swap_tensor_on:
            x = swap_tensor_wrapper(x, tag="ffn_residual", group_swap=self.group_swap, cpu_pool=self.cpu_pool)
        x = self.norm2(x)
        return x
...
def _build_swap_tensor_model(mode: str, cpu_pool=None) -> torch.nn.Module:
    """Build and configure a model using the swap_tensor_wrapper() interface.

    Args:
        mode: One of 'none', 'swap_tensor', 'swap_tensor_with_pool', or 'group_swap'.
        cpu_pool: Optional pinned host memory pool handed to swap_tensor_wrapper().
    ...
    if mode in ("swap_tensor", "swap_tensor_with_pool", "group_swap"):
        model = _SwapTensorTransformer(
            ...
            cpu_pool=cpu_pool if mode == "swap_tensor_with_pool" else None,
        ).npu()
        for i in range(len(model.layers) - 1):
            SwapManager().set_forward_prefetch_layer(model.layers[i], model.layers[i + 1])
        return model
...
def test_act_swap_tensor_function_mode():
    ...
    Description: Validate the declarative tensor swap interface by comparing
                 training across four modes: 'none' (baseline),
                 'swap_tensor' (manually offload selected transformer activations
                 via swap_tensor_wrapper()), 'swap_tensor_with_pool' (same, but
                 each offload draws from an explicit PinnedMemoryPool), and
                 'group_swap' (offload the same activations with group copy
                 fusion enabled).
    ...
    Expectation: All modes produce consistent losses (within 1e-5 tolerance),
                 and NONE > SWAP_TENSOR, NONE > SWAP_TENSOR_WITH_POOL and
                 NONE > GROUP_SWAP in peak device memory. The explicit pool is
                 populated (total_allocated > 0) by the pooled mode.
    """
    print("Starting swap_tensor_wrapper() comparison: none vs swap_tensor vs swap_tensor_with_pool vs group_swap")
    ...
    modes = ["none", "swap_tensor", "swap_tensor_with_pool", "group_swap"]
    # Deep enough that the pool-mode offloads stay low on device while buffering
    # a few layers' worth of pinned host memory at once.
    cpu_pool = PinnedMemoryPool(max_host_bytes=512 * 1024**2)
    ...
    for mode in modes:
        print(f"\n--- Running mode: {mode.upper()} ---")
        with seed_memory_time_context() as stats:
            model = _build_swap_tensor_model(mode, cpu_pool=cpu_pool)
            losses = train_one_mode(model, dataloader, train_steps)
        peak_mem = stats.get("peak_mem")
        duration = stats.get("exec_time")
        results[mode] = {"losses": losses, "peak_mem_gb": peak_mem, "time_sec": duration}
    ...
    for step in range(train_steps):
        base_val = base_losses[step]
        for mode in ["swap_tensor", "swap_tensor_with_pool", "group_swap"]:
            val = results[mode]["losses"][step]
            diff = abs(val - base_val)
            assert diff < tol, (
                f"Loss mismatch at step {step} in mode '{mode}': "
    ...
    assert cpu_pool.total_allocated > 0, \
        "Expected swap_tensor_wrapper to allocate from the explicit CPU pool."
...
class _CpuPoolSwapBlock(nn.Module):
    """Generate many activations saved independently for backward."""

    def __init__(self, dim: int, activation_count: int) -> None:
        """Initialize a block that creates the requested number of activations."""
        super().__init__()
        self.bias = nn.Parameter(torch.randn(dim) * 0.01)
        self.activation_count = activation_count

    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        """Apply a sequence of operations whose outputs are saved for backward."""
        for _ in range(self.activation_count):
            inputs = torch.relu(inputs + self.bias)
        return inputs


class _CpuPoolSwapNet(nn.Module):
    """Two-layer network that enables swap scheduling for the first layer."""

    def __init__(self, dim: int, activation_count: int, cpu_pool: Optional[PinnedMemoryPool] = None) -> None:
        """Wrap the activation-producing block with per-tensor activation swap."""
        super().__init__()
        swap_block = swap_wrapper(
            _CpuPoolSwapBlock(dim, activation_count),
            policy_fn=lambda _: CheckpointPolicy.MUST_SWAP,
            cpu_pool=cpu_pool,
            group_swap=True
        )
        ...
        SwapManager().set_forward_prefetch_layer(self.layers[0], self.layers[1])

    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        """Run the swappable layer followed by the scheduling boundary."""
        for layer in self.layers:
            inputs = layer(inputs)
        return inputs.square().mean()


def _run_cpu_pool_swap_step(
    cpu_pool: Optional[PinnedMemoryPool],
    seed: int,
    steps: int = 5,
    warmup_steps: int = 3,
) -> tuple[list[float], list[float]]:
    """Run synchronized per-tensor swap steps and return per-step metrics."""
    if steps <= 0:
        raise ValueError(f"steps must be positive, but got {steps}")
    if warmup_steps < 0:
        raise ValueError(f"warmup_steps must be non-negative, but got {warmup_steps}")

    set_seed(seed)
    model = _CpuPoolSwapNet(dim=1024, activation_count=256, cpu_pool=cpu_pool).npu()
    optimizer = torch.optim.SGD(model.parameters(), lr=1e-3)
    inputs = [torch.randn(128, 1024).npu().requires_grad_() for _ in range(warmup_steps + steps)]
    losses = []
    step_times_ms = []

    for step, step_input in enumerate(inputs):
        torch.npu.synchronize()
        start = time.perf_counter()
        optimizer.zero_grad(set_to_none=True)
        loss = model(step_input)
        loss.backward()
        optimizer.step()
        torch.npu.synchronize()
    ...
    del inputs, loss, optimizer, model
    torch.npu.empty_cache()
    return losses, step_times_ms


def test_act_swap_tensor_cpu_pool_performance():
    """
    Feature: Pinned CPU pool reuse for per-tensor activation swap
    Description: Use swap_wrapper with a MUST_SWAP per-tensor policy, warm both paths,
                 then compare identical activation-swap steps with and without pool reuse.
                 Synchronize NPU work at both timing boundaries.
    Expectation: Every loss pair aligns within 1e-5, the explicit pool is populated,
                 and its average measured step time is shorter than the non-pool path.
    """
    steps = 10
    cpu_pool = PinnedMemoryPool(max_host_bytes=256 * 1024**2)

    with_pool_losses, with_pool_step_times = _run_cpu_pool_swap_step(cpu_pool, seed=42, steps=steps)
    without_pool_losses, without_pool_step_times = _run_cpu_pool_swap_step(None, seed=42, steps=steps)

    assert len(without_pool_losses) == steps
    assert len(with_pool_losses) == steps
    assert len(without_pool_step_times) == steps
    assert len(with_pool_step_times) == steps
    assert all(step_time > 0 for step_time in without_pool_step_times + with_pool_step_times)
    assert cpu_pool.total_allocated > 0, "Expected swap_wrapper to allocate from the explicit CPU pool."

    for step, (without_pool_loss, with_pool_loss) in enumerate(zip(without_pool_losses, with_pool_losses)):
        loss_diff = abs(with_pool_loss - without_pool_loss)
        assert loss_diff < 1e-5, (
            f"Loss mismatch at step {step}: without_cpu_pool={without_pool_loss:.8f}, "
            f"with_cpu_pool={with_pool_loss:.8f}, diff={loss_diff:.2e}"
        )

    without_pool_avg_ms = sum(without_pool_step_times) / steps
    with_pool_avg_ms = sum(with_pool_step_times) / steps
    print(
        "Per-tensor activation swap CPU pool comparison passed: "
        f"without_cpu_pool_step_ms={[round(value, 3) for value in without_pool_step_times]}, "
        f"with_cpu_pool_step_ms={[round(value, 3) for value in with_pool_step_times]}, "
        f"without_cpu_pool_avg_ms={without_pool_avg_ms:.3f}, "
        f"with_cpu_pool_avg_ms={with_pool_avg_ms:.3f}, "
        f"pool_allocated_bytes={cpu_pool.total_allocated}"
    )
