...
class TestOptimizerSwapBuildPath(unittest.TestCase):
    ...
    @arg_mark(**_MARK)
    def test_unit_pipeline_conflict_fails_fast(self):
        """Feature: optimizer.swap against the unit-pipelined step.
        Description: enable swap while ``HP_OFFLOAD_UNIT_PIPELINE`` is on.
        Expectation: the build fails instead of stepping past the swap pipeline.
        """
        config = _trainer_config(
            _builder_target(torch.optim.AdamW(self.model.parameters(), lr=0.01)),
            OptimizerSwapConfig(enabled=True),
        )

        with mock.patch.dict(os.environ, {fsdp_runtime.UNIT_PIPELINE_ENV: "1"}):
            with self.assertRaisesRegex(ValueError, fsdp_runtime.UNIT_PIPELINE_ENV):
                _build_optimizer(config, self.model)
