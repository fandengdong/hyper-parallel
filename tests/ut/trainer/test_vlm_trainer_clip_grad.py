...
Clipping itself now lives in ``BaseTrainer.prepare_optimizer_step`` (shared by
the text and VLM steps), so the real implementation is bound into the stub and
``hyper_parallel.trainer.base.clip_grad_norm_`` is the patched target.

One deliberate exception is pinned below: model-integration diagnostics report
the pre-clip norm, so when ``runtime_enabled`` is on and clipping is switched
off, the step still *measures* the norm with an infinite threshold. Without the
runtime it stays free.
"""
...
def _build_trainer(max_grad_norm, runtime_enabled=False):
    ...
    """
    from hyper_parallel.trainer.base import BaseTrainer  # pylint: disable=C0415
    ...
    integration = SimpleNamespace(
        ...
        after_optimizer=lambda *_args, **_kwargs: None,
        after_backward_before_clip=lambda *_args, **_kwargs: None,
        after_clip=lambda *_args, **_kwargs: None,
    )
    ...
    base = SimpleNamespace(
        ...
        begin_fsdp_runtime_diagnostics=lambda _micro_step: None,
        validate_fsdp_runtime_before_optimizer=lambda: None,
        _end_model_integration_step=lambda _metrics: None,
        forward_backward_step=lambda _model_inputs, _loss_inputs: (
            torch.tensor(1.0),
    ...
    # Exercise the production clipping path rather than a stub of it.
    base.prepare_optimizer_step = BaseTrainer.prepare_optimizer_step.__get__(base)
    trainer.base = base
    return trainer, reported


def _run_step(max_grad_norm, clip_result, runtime_enabled=False):
    """Run one stubbed VLM step and return the step result, reports and clip mock."""
    trainer, reported = _build_trainer(max_grad_norm, runtime_enabled=runtime_enabled)
...
class TestVlmTrainerGradientClip(unittest.TestCase):
    ...
    @arg_mark(plat_marks=["cpu_linux", "cpu_macos"], level_mark="level0",
              card_mark="allcards", essential_mark="essential")
    def test_runtime_diagnostics_measure_the_norm_with_clipping_off(self):
        """Model diagnostics still report the norm, measured with an infinite threshold.

        This is the documented cost of the opt-in runtime: with clipping off and
        no runtime the norm is never computed, but the runtime needs it to feed
        ``after_clip``, so the call happens with ``inf`` (measure-only).
        """
        result, reported, clip, trainer = _run_step(
            0.0, torch.tensor(0.5), runtime_enabled=True
        )

        clip.assert_called_once_with(trainer.base.model, float("inf"))
        self.assertAlmostEqual(result["grad_norm"], 0.5)
        self.assertAlmostEqual(reported["grad_norm"], 0.5)
