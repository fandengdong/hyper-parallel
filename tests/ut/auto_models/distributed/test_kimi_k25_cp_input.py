...
from hyper_parallel.models.kimi_k25.adapter.distributed.context_parallel import (
...
class _FakeModel(nn.Module):
    ...
    def __init__(self, model_type="kimi_k25", with_language_model=True):
...
class _FakeMesh:
    ...
    def __init__(self, cp_size, cp_rank=0):
...
class TestBindContextParallel(unittest.TestCase):
    ...
    def test_cp1_is_a_no_op(self):
        ...
        self.assertFalse(getattr(language_model, "_hp_kimi_k25_cp_sharded", False))
    ...
    def test_rejects_unexpected_model_structure(self):
        """A non-kimi_k25 config or a missing text tower fails fast."""
        with self.assertRaisesRegex(TypeError, "model_type='kimi_k25'"):
