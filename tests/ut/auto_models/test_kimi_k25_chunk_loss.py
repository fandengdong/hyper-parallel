...
from hyper_parallel.models.kimi_k25.adapter.chunk_loss import (  # pylint: disable=wrong-import-position
    bind_chunk_loss,
)
...
class TestKimiK25ChunkLoss(unittest.TestCase):
    ...
    def test_chunked_loss_matches_full_logits_loss_and_gradients(self) -> None:
        ...
        loss_inputs = {"labels": labels, "loss_mask": labels.ne(IGNORE_INDEX)}
    ...
    def test_prepare_model_inputs_shifts_raw_labels(self) -> None:
        """Raw VLM labels must be shifted onto one target per hidden position."""
        input_ids, labels = self._inputs()
        model = _tiny_model()
        loss_fn = ChunkedCausalLMLoss(chunk_size=4, ignore_index=IGNORE_INDEX)
        loss_fn.bind_model(model, _single_rank_setup())

        prepared = loss_fn.prepare_model_inputs(
            {"input_ids": input_ids, "labels": labels},
            {"labels": labels, "loss_mask": labels.ne(IGNORE_INDEX)},
        )

        targets = prepared["chunk_loss_targets"]
        self.assertEqual(tuple(targets.shape), (1, SEQ_LEN))
        torch.testing.assert_close(targets[0, :-1], labels[0, 1:])
        self.assertEqual(int(targets[0, -1].item()), IGNORE_INDEX)

        mask = prepared["chunk_loss_mask"]
        self.assertEqual(mask.dtype, torch.bool)
        torch.testing.assert_close(mask[0, :-1], labels[0, 1:].ne(IGNORE_INDEX))
    ...
    def test_bind_rejects_foreign_model_family(self) -> None:
        ...
        model = _tiny_model()
        model.config.model_type = "not_kimi_k25"
