...
class TestShardedDtensorConstruction(unittest.TestCase):
    ...
    def test_cached_layout_path_matches_the_rebuilding_path(self):
        ...
        with patch("hyper_parallel.core.fully_shard.hsdp_param._FAST_SHARDED_DTENSOR", False):
            rebuilt = hsdp_param.to_sharded_dtensor(local)
        with patch("hyper_parallel.core.fully_shard.hsdp_param._FAST_SHARDED_DTENSOR", True):
