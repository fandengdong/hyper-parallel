...
class TestAsyncSaveOnCpu(unittest.TestCase):
    ...
    def test_pack_hook_keeps_the_original_and_storage_aliases_it(self):
        """``pack_hook`` returns the original tensor while storage aliases its data.

        The saved tensor has to stay the original object: returning a detached
        alias would trap input gradients on the alias, so upstream modules (and
        their FSDP post-backward/reduce) are never reached.  The ``SwapTensor``
        keeps its own detached alias purely for offload bookkeeping, and that
        alias shares storage, so offload/restore act on the saved tensor too.
        """
        ...
        aliased = saved_tensors.storage[0][0].val
        self.assertIsNot(aliased, packed)
        self.assertEqual(
            aliased.untyped_storage().data_ptr(), packed.untyped_storage().data_ptr()
        )
