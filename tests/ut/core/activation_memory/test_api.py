...
class TestCheckpointFunction(unittest.TestCase):
    ...
    def test_checkpoint_with_swap_inputs(self):
        """Test checkpoint with swap_inputs=True enters the native save-on-cpu context."""
        def _dummy_fn(x):
            return x * 2

        with _spy_checkpoint(), patch(
            "hyper_parallel.core.activation_memory.api.native_save_on_cpu",
    ...
    def test_checkpoint_swap_inputs_uses_native_context_whatever_the_pool_knobs(self):
        """Test input swapping enters the native context, ignoring the policy knobs.

        ``group_swap``/``cpu_pool`` configure the selective-policy swap path; the
        boundary-input swap runs through ``native_save_on_cpu`` because the
        SwapManager path's ``resize_(0)`` corrupts the backward pass on NPU.
        """
        events = []
        ...
        mock_native.assert_called_once_with()
        self.assertIsNotNone(spy.call_args)
