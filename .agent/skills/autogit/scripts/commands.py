...
def _raise_pr_gates_non_interactive_error(undecided_keys: List[str]) -> None:
    ...
    lines = [
        ...
        "Ask the user one gate at a time in chat, then re-invoke",
        "autogit with all flags in a single call.",
        "",
...
def _preview_and_confirm(label: str, content: str,
    ...
    if not sys.stdin.isatty():
        ...
        raise AutoGitError(
            ...
            f"Generating a high-quality {label} requires reading the diff\n"
            f"and producing natural-language\n"
