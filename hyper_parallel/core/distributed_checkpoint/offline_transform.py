...
from hyper_parallel.core.distributed_checkpoint.standard_planner import (
    StandardSavePlanner,
    _DcpMergeLoadPlanner,
)
from hyper_parallel.core.distributed_checkpoint.checkpoint_io import (
    load_checkpoint_file,
    save_checkpoint_file,
)
from hyper_parallel.core.distributed_checkpoint.utils import get_tensor_storage_size
...
def _validate_state_dict(state_dict: dict[str, Any]) -> None:
    """Ensure values are allowed before DCP / HF export.

    Applies to a full-weights ``state_dict`` (from a full checkpoint file or merged shard checkpoint content).

    Allowed values are ``torch.Tensor`` (including ``nn.Parameter``), ``bytes`` (e.g. optimizer blobs), or
    ``str`` placeholders (e.g. bitsandbytes serialization per HF ecosystem).
...
def _split_state_dict_into_shards(
    ...
    """Shard ``state_dict`` for safetensors by serialized tensor byte size.
...
def save_state_dict_as_huggingface_format(
    ...
    Args:
        ...
        state_dict: Parameter tensors keyed by name.
...
def parse_checkpoint_from_huggingface(resume_from_checkpoint: str | os.PathLike[str]) -> dict[str, Any]:
    ...
    Returns:
        Flattened str-keyed state dict (tensors on CPU when applicable).
    """
...
def _full_checkpoint_format_for_path(path: str) -> str:
    """Infer the :func:`load_checkpoint_file` ``ckpt_format`` from a full-checkpoint file suffix."""
    lower = path.lower()
...
def full_state_dict_to_dcp_format(
    ...
    Args:
        state_dict: Merged weights to store (see project conventions for the accepted value types).
...
def convert_full_checkpoint_to_dcp(
    ...
    dst_dir: str | os.PathLike[str],
    src_platform: Literal["torch", "huggingface"] = "torch",
) -> None:
    """Load checkpoint weights and write HyperParallel DCP under ``dst_dir``.

    Args:
        src_ckpt: Source path: Hugging Face safetensors directory (full or shard layout), or a PyTorch
            **full checkpoint** file (pickle or ``.safetensors``).
        dst_dir: Output directory for DCP files.
        src_platform: ``huggingface`` (HF directory) or ``torch`` (full-checkpoint file read by
            :func:`load_checkpoint_file`).
