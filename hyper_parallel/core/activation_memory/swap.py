    ...
    def launch_offload(self, copy_stream):
        ...
        if _SWAP_SYNC_COPY:
            ...
            with _backend.no_grad():
                if total_bytes > 0:
                    group_device_bufs = self._pack_device_buffers()
                    group_cpu_bufs = {}
                    for bucket_key, bucket in self._packed_buckets.items():
                        cpu_buf = self._acquire_bucket_cpu_buf(bucket_key, bucket)
                        group_cpu_bufs[bucket_key] = cpu_buf
                        cpu_buf[:bucket["total_numel"]].copy_(
                            group_device_bufs[bucket_key], non_blocking=False
                        )
...
class SwapManager:
    ...
    def set_forward_prefetch_layer(self, first_layer, second_layer, tensor_backward_hooks: bool = False):
        ...
        def _first_grad_tensor(value):
            ...
            def _collect(x):
                if isinstance(x, _backend.Tensor) and x.requires_grad and not found:
                    found.append(x)
                return x

            _backend.tree_map(_collect, value)
    ...
    def _get_copy_stream(self):
        ...
        if _SWAP_SYNC_COPY:
            ...
            return _backend.get_current_stream()
