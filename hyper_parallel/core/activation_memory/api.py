...
def native_save_on_cpu(pin_memory: bool = True) -> Any:
    """Return the context that moves saved tensors to (pinned) host memory.

    Unlike the ``SwapManager``/async path behind :func:`async_save_on_cpu`, the device
    original is released by the allocator's normal refcount and stream tracking instead
    of an explicit ``storage.resize_(0)``, which deterministically corrupts the backward
    pass on NPU (every parameter gradient becomes NaN).

    The device-to-host copy is non-blocking: the pinned destination is retained by the
    checkpoint frame until backward, and the device source is released by the allocator's
    stream-aware free path, so the copy overlaps compute instead of stalling the
    host/device pipeline once per checkpointed block.

    Args:
        pin_memory: Whether to stage the saved tensors in pinned host memory.

    Returns:
        A ``saved_tensors_hooks`` context manager that swaps saved tensors to host.
    """
    device_type = "npu" if (hasattr(torch, "npu") and torch.npu.is_available()) else "cuda"
    device_module = getattr(torch, device_type, torch.cuda)

    def pack_to_cpu(tensor: Any) -> Any:
        """Move one saved tensor to host memory, pinned when requested."""
        if not pin_memory:
    ...
    def unpack_from_cpu(packed: Any) -> Any:
        """Move a saved tensor back to the device it was saved from."""
