...
Provides model state dict get/set (``get_model_state_dict``,
``set_model_state_dict``) and optimizer state dict get/set
(``get_optim_state_dict``, ``set_optim_state_dict``) that handle HSDP
DTensor parameters, FQN-based keys, full/local/CPU conversion,
broadcast-from-rank0, flatten/unflatten, and DCP load template construction.

The optimizer state dict functions do **not** call PyTorch DCP's
