...
# ============================================================================
"""Kimi-K2.6 adapter for model-integrated Chunk Loss training."""
...
# This family adapter targets the PyTorch-native Transformers Kimi-K2.6
...
def _align_chunk_loss_inputs(
    ...
    if targets.dim() != 2 or hidden_states.shape[:2] != targets.shape:
        raise ValueError(
            "Kimi-K2.6 Chunk Loss targets must match final hidden batch/sequence "
            f"dimensions, got hidden={tuple(hidden_states.shape)} and "
            f"targets={tuple(targets.shape)}"
        )
    if loss_mask is not None and loss_mask.shape != targets.shape:
        raise ValueError("Kimi-K2.6 Chunk Loss mask must match targets")
...
def _kimi_k25_chunk_loss_forward(
    model: nn.Module,
    *,
    ...
    **kwargs: Any,
    ...
    """Run Kimi-K2.6 through final hidden states and skip full logits."""
    if past_key_values is not None or use_cache:
        raise ValueError("Kimi-K2.6 Chunk Loss training does not support KV cache")
    if isinstance(logits_to_keep, torch.Tensor) or logits_to_keep != 0:
        raise ValueError("Kimi-K2.6 Chunk Loss requires logits_to_keep=0")
    if kwargs.get("return_dict") is False:
        raise ValueError("Kimi-K2.6 Chunk Loss requires return_dict=True")
...
def bind_chunk_loss(model: nn.Module) -> None:
    """Atomically install the Kimi-K2.6 Chunk Loss training forward."""
    if getattr(model, "_hp_kimi_k25_chunk_loss_bound", False):
        return
    model_type = getattr(getattr(model, "config", None), "model_type", None)
    if model_type != "kimi_k25":
        raise TypeError(
            "Kimi-K2.6 Chunk Loss adapter requires config.model_type='kimi_k25', "
            f"got {model_type!r}"
        )
    if not isinstance(getattr(model, "model", None), nn.Module):
        raise TypeError("Kimi-K2.6 Chunk Loss adapter requires a decoder module at model.model")
    lm_head = getattr(model, "lm_head", None)
    if not isinstance(lm_head, nn.Linear) or lm_head.bias is not None:
        raise TypeError("Kimi-K2.6 Chunk Loss adapter requires a bias-free nn.Linear lm_head")
    ...
    def _chunk_loss_aware_forward(*args: Any, **kwargs: Any) -> Any:
        ...
        if args:
            raise TypeError(
                "Kimi-K2.6 Chunk Loss Trainer integration passes model inputs by keyword"
            )
        return _kimi_k25_chunk_loss_forward(model, **kwargs)

    _commit_forward_rewrite(
        _ForwardRewriteRequest(
            model,
            _chunk_loss_aware_forward,
            companion_attrs={"_hp_kimi_k25_chunk_loss_bound": True},
        )
    )
