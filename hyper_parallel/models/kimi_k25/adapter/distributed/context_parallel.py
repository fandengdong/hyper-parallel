...
_CP_SHARDED_FLAG = "_hp_kimi_k25_cp_sharded"
...
def bind_context_parallel(model: nn.Module, mesh_context: Any) -> None:
    ...
    Raises:
        TypeError: If the model does not expose the expected kimi_k25 structure.
    """
    ...
    if model_type != "kimi_k25":
        raise TypeError(
            "Kimi-K2.6 CP input sharding requires config.model_type='kimi_k25', "
            f"got {model_type!r}"
        )
